...
import { AdvisoryRequestStatus } from '../interfaces/admin/advisoryRequest.interface';

/**
 * Shared validation schema for Advisory Request
 * Used by Frontend (Formik) with native validationSchema support
 */
export const CreateAdvisoryRequestSchema = yup.object({
  number: yup
    .string()
    .matches(/^ADV\d{7}$/, 'Invalid ticket number format')
    .max(16),
  ...
  status: yup
    .string()
    .oneOf(Object.values(AdvisoryRequestStatus))
    .default(AdvisoryRequestStatus.NEW),
  assignmentGroup: yup
    .string()
...
/**
 * Schema for saving an Advisory Request as draft (only caller and createdBy required)
 */
export const DraftAdvisoryRequestSchema = yup.object({
  number: yup
    .string()
    .matches(/^ADV\d{7}$/, 'Invalid ticket number format')
    .max(16),
  ...
  status: yup
    .string()
    .oneOf(Object.values(AdvisoryRequestStatus))
    .default(AdvisoryRequestStatus.DRAFT),
...
/**
 * Schema for updating an Advisory Request entry (all fields optional)
 */
export const UpdateAdvisoryRequestSchema = yup.object({
  number: yup
    .string()
    .matches(/^ADV\d{7}$/, 'Invalid ticket number format')
    .max(16),
  ...
  status: yup
    .string()
    .oneOf([...Object.values(AdvisoryRequestStatus), undefined as unknown as string])
    .optional(),
...
/**
 * Schema for validating Advisory Request ID parameter (integer)
 */
export const AdvisoryRequestIdSchema = yup.object({
 ...
 */
export type CreateAdvisoryRequestDto = yup.InferType<typeof CreateAdvisoryRequestSchema>;
export type UpdateAdvisoryRequestDto = Partial<CreateAdvisoryRequestDto>;

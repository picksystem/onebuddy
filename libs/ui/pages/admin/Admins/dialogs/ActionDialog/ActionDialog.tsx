...
interface ActionDialogProps {
  actionTarget: { user: AdminRow; type: ActionType } | null;
  actionNotes: string;
...
const ActionDialog = ({
  actionTarget, actionNotes, actionInProgress,
  onClose, onNotesChange, onConfirm,
}: ActionDialogProps) => {
        ...
        <>
          <Box className={isApprove ? classes.dialogHeaderApprove : classes.dialogHeaderReject}>
            <Box className={classes.dialogHeaderBadge}>
              ...
              >
                {isApprove ? 'Approve Admin Request' : 'Reject Admin Request'}
              </Typography>
            </Box>
            <Box className={classes.dialogHeaderUserRow}>
              ...
              <Box className={classes.dialogHeaderUserInfo}>
                <Typography variant='subtitle1' fontWeight={700} className={classes.dialogHeaderName}>
          ...
          <DialogContent sx={{ p: 3 }}>
            <Typography variant='body2' color='text.secondary' className={classes.descriptionText}>
              {isApprove
                ? `You are about to approve admin access for ${actionTarget.user.name}. They will have full administrative privileges.`
                : `You are about to reject the admin request from ${actionTarget.user.name}. They will not be granted admin access.`}
            </Typography>
            <TextField
              ...
              fullWidth
              placeholder={
                isApprove ? 'Add any notes about this approval...' : 'Provide a reason for rejection...'
              }
              value={actionNotes}
